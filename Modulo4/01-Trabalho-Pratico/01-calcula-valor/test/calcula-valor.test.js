...
describe('calcularPrestacoes', () => {
  ...
  test('O montante deve ser maior que zero', () => {
    const numeroPrestacoes = 3;
    const montante = -100;
    ...
    expect(prestacoes).toHaveLength(0);
    expect(prestacoes).not.toHaveLength(numeroPrestacoes);
  });

  test('O Número de parcelas deve ser um numero inteiro maior que zero', () => {
    const numeroPrestacoes = -2;
    const montante = 100;
    ...
    expect(prestacoes.length).toBe(0);
    expect(prestacoes).toHaveLength(0);
    expect(prestacoes).not.toHaveLength(3);
  });
});
